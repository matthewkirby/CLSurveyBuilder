...
class Converter(object):
    ...
    def setup_splines(self):
        ...
        fit_lssmock = np.loadtxt(os.path.join(os.environ['RICHESTRMCL'], fname))
    ...
    def setup_iCDF_grid(self):
        ...
        try:
            gridpath = os.path.join(os.environ['HMFGRIDPATH'], 'projection_model.pkl')
        except KeyError:
            gridpath = 'grids/projection_model.pkl'

        try:
            with open(gridpath, "rb") as input_file:
                # output_obj = cPickle.load(input_file)
                output_obj = pickle.load(input_file)#, encoding='latin1')
    ...
    def build_iCDF_grid(self):
        ...
        gridpath = os.path.join(os.environ['HMFGRIDPATH'], 'projection_model.pkl')
        with open(gridpath, "wb") as output_file:
...
def draw_observed_mgas(catalog, model):
    """Draw adjustments for the true observables to convert to observed quantities
    ...
    Returns
    -------
    Dataframe with columns for the observed gas mass
    """
    stdobs_mgas = model['stdobs_lnmgas']*catalog['mean_mgas']
    obs_mgas_adjustment = np.random.normal(0.0, stdobs_mgas)
    catalog['obs_mgas'] = catalog['true_mgas'] + obs_mgas_adjustment
    catalog['obs_mgas_std'] = stdobs_mgas

    return catalog


def draw_observed_richness(catalog, drawer):
...
def draw_observed_properties(catalog, model, drawer, mgas_rescale=1.0):
    """ Draw observed properties for a given mass catalog

    Using this function instead of the individual calls for
    richness and gas mass allows us to filter the cases
    when we do not care about gas mass.
    ...
    model : dict
        Model parameters
    drawer : Converter(object)
        An initialized converter to not have to regen it for 
        multiple catalogs
    mgas_rescale : float, optional
        Units that gas mass was drawn in. We don't want our output
        catalogs have units of 1e12 Msun (with/without hinv)

    Returns
    -------
    Dataframe with new columns for the observed properties
    """
    if 'true_mgas' in catalog.columns:
        draw_observed_mgas(catalog, model)
        catalog['obs_mgas'] *= mgas_rescale
        catalog['obs_mgas_std'] *= mgas_rescale
    t1 = time()
    catalog = draw_observed_richness(catalog, drawer)
    t2 = time()
    print("observed richness draws took {:.3f}s".format(t2-t1))
    return catalog
...
if __name__ == "__main__":
    x = Converter()
    x.setup_splines()
    ltlist = np.hstack((np.linspace(1,100,50), np.linspace(100,2000,100)[1:]))
    z = 0.22

    model = {'lam': ltlist,
             'fprj': x.fprjfit(ltlist,z),
             'fmsk': x.fmaskfit(ltlist,z),
             'mu': x.muprjfit(ltlist,z),
             'sigma': x.sigprjfit(ltlist,z),
             'tau': x.tauprjfit(ltlist,z)
            }

    outputs = np.array([model['lam'], model['fprj'], model['fmsk'], model['mu'],
                        model['sigma'], model['tau']])
    np.savetxt('../extended_projection_model.dat', outputs.transpose())
